...
const BACKGROUND = __inGameColor(10, 10, 10);
const WALL = __inGameColor(127, 127, 127);
const BEDROCK = __inGameColor(99, 99, 99);
const SAND = __inGameColor(223, 193, 99);
const SNOW = __inGameColor(245, 245, 245);
const WATER = __inGameColor(0, 10, 255);
const PLANT = __inGameColor(0, 220, 0);
const FIRE = __inGameColor(255, 0, 10);
const FIRE_CURSE = __inGameColor(36, 240, 50);
const FIRE_BRIGHT = __inGameColor(230, 230, 230);
const FIRE_BLUE = __inGameColor(5, 15, 255);
...
const FIREWORK = __inGameColor(129, 169, 184);
...
const CORRUPT = __inGameColor(117, 56, 161);
...
const HUMAN = __inGameColor(237, 197, 128);
const THANOS = __inGameColor(184, 54, 217);
const DUST = __inGameColor(199, 199, 199);
const NANITES = __inGameColor(65, 138, 186);
const SPICE = __inGameColor(255, 64, 0);
const HUMAN_MAKER = __inGameColor(173, 145, 95);
const CLONE = __inGameColor(252, 154, 216);
...
function initElements() {
	...
	for (var i = 0; i < elements.length; i++) {
		...
		const color_idx = ((color & 0x30000) >>> 12) + ((color & 0x300) >>> 6) + (color & 0x3);
		if (color_idx !== i) throw "elements array order does not match element indices";
...
function SNOW_ACTION(x, y, i) {
  ...
  /* Heat Wave modifier */
  if (HW_ENABLED === true) {
		if (random() < 1) gameImagedata32[i] = WATER;
	}
...
function WATER_ACTION(x, y, i) {
	...
	if (doDensityLiquid(x, y, i, SNOW, 25, 50)) return;
	if (doDensityLiquid(x, y, i, HUMAN, 7, 30)) return;
	if (__doCorruption(x, y, i)) return;
	if (HW_ENABLED === true) {
		if (random() < 5) gameImagedata32[i] = STEAM;
	}
	if (CS_ENABLED === true) {
		if (random() < 30) gameImagedata32[i] = CHILLED_ICE;
	}
}

function PLANT_ACTION(x, y, i) {
	doGrow(x, y, i, WATER, 50);
	doGrow(x, y, i, CLONE, 10)
	if (random() < 5) {
...
function FIRE_ACTION(x, y, i) {
	...
	if (random() < 1) {
		...
		if (waxLoc !== -1) {
			...
			const fallLoc = below(Math.max(y, waxXY[1]), Math.max(i, waxLoc), BACKGROUND);
	...
	var flameOutChance = 40;
	if (HW_ENABLED === true) {
		flameOutChance = 25;
	}
	if (random() < flameOutChance) {
		var flameOut = true;
		...
		for (yIter = yStart; yIter !== yEnd; yIter++) {
			...
			for (xIter = xStart; xIter !== xEnd; xIter++) {
				...
				if (borderingElem === PLANT || borderingElem === FUSE || borderingElem === BRANCH || borderingElem === LEAF || borderingElem === MYSTERY) {
					flameOut = false;
					break;
				}
				/*
...
function FIRE_CURSE_ACTION(x, y, i) {
	...
	if (random() < 10) {
		...
		if (waxLoc !== -1) {
			const waxXY = fastItoXYBordering(x, y, i, waxLoc);
			gameImagedata32[waxLoc] = FIRE_CURSE;
			const fallLoc = below(Math.max(y, waxXY[1]), Math.max(i, waxLoc), BACKGROUND);
			if (fallLoc !== -1) gameImagedata32[fallLoc] = FALLING_WAX;
			return;
		}
	...
	if (random() < flameOutChance) {
		...
		for (yIter = yStart; yIter !== yEnd; yIter++) {
			...
			for (xIter = xStart; xIter !== xEnd; xIter++) {
				...
				const idx = idxBase + xIter;
				const borderingElem = gameImagedata32[idx];
				if (borderingElem === FIRE_CURSE) continue;
				if (borderingElem === BACKGROUND || borderingElem === WATER || borderingElem === SALT_WATER || borderingElem === STEAM || borderingElem === LAVA || borderingElem === ACID || borderingElem === BEDROCK || borderingElem === DEEPSAND || borderingElem === VOID) {
					flameOut = true;
				} else {
					if (random() < 60) {
						gameImagedata32[idx] = FIRE_CURSE;
						flameOut = false;
						break;
					}
				}
	...
	/* rising fire */
	if (fireRise(x, y, i, FIRE_CURSE)) return;
}
...
function FIRE_BRIGHT_ACTION(x, y, i) {
	...
	if (colorFire(x, y, i, 50, true)) return;
	if (fireRise(x, y, i, FIRE_BRIGHT)) return;
}
...
function FIRE_BLUE_ACTION(x, y, i) {
	...
	if (colorFire(x, y, i, 50, true)) return;
	if (fireRise(x, y, i, FIRE_BLUE)) return;
}
...
function SALT_WATER_ACTION(x, y, i) {
	...
	if (doDensityLiquid(x, y, i, HUMAN, 5, 30)) return;
	if (__doCorruption(x, y, i)) return;
	if (HW_ENABLED === true) {
		if (random() < 20) gameImagedata32[i] = STEAM;
	}
	if (CS_ENABLED === true) {
		if (random() < 20) gameImagedata32[i] = CHILLED_ICE;
	}
}
...
function GUNPOWDER_ACTION(x, y, i) {
	if (random() < 95) {
		if ((bordering(x, y, i, FIRE) !== -1) || (bordering(x, y, i, FIRE_CURSE) !== -1) || (II_ENABLED === true)) {
...
function FIREWORK_ACTION(x, y, i) {
	...
	if (random() < 20) {
		if ((bordering(x, y, i, FIRE) !== -1) || (bordering(x, y, i, FIRE_CURSE) !== -1) || (II_ENABLED === true)) {
      if (random() < 50) {
  			particles.addActiveParticle(FIREWORK_PARTICLE, x, y, i);
  			gameImagedata32[i] = BACKGROUND;
  			return;
      } else {
        gameImagedata32[i] = BACKGROUND;
...
function NITRO_ACTION(x, y, i) {
	...
	if ((borderingAdjacent(x, y, i, FIRE) !== -1) || (borderingAdjacent(x, y, i, FIRE_CURSE) !== -1) || (II_ENABLED === true)) {
		if (random() < 30) {
...
function NAPALM_ACTION(x, y, i) {
	if ((random() < 25 && bordering(x, y, i, FIRE) !== -1) || (random() < 25 && bordering(x, y, i, FIRE_CURSE) !== -1) || (II_ENABLED === true)) {
...
function C4_ACTION(x, y, i) {
	if ((random() < 60 && bordering(x, y, i, FIRE) !== -1) || (random() < 60 && bordering(x, y, i, FIRE_CURSE) !== -1) || (II_ENABLED === true)) {
...
function ICE_ACTION(x, y, i) {
	...
	if (__doCorruption(x, y, i)) return;
	if (HW_ENABLED === true) {
		if (random() < 5) gameImagedata32[i] = WATER;
	}
}
...
function CHILLED_ICE_ACTION(x, y, i) {
	...
	if (__doCorruption(x, y, i)) return;
	if (HW_ENABLED === true) {
		if (random() < 15) gameImagedata32[i] = WATER;
	}
	/*
	 * Check the four surrounding pixels (up, down, left, right)
	 * for something that will make us thaw faster
	 */
	if (bordering(x, y, i, SALT) !== -1 || bordering(x, y, i, SALT_WATER) !== -1 || bordering(x, y, i, LAVA) !== -1 || bordering(x, y, i, FIRE) !== -1 || bordering(x, y, i, STEAM) !== -1) {
		gameImagedata32[i] = ICE;
		return;
	}
	doGrow(x, y, i, WATER, 50);
...
function LAVA_ACTION(x, y, i) {
	...
	else if (left !== -1 && gameImagedata32[left] !== LAVA && gameImagedata32[left] !== BACKGROUND) skipDirectAdjacent = false;
	else if (right !== -1 && gameImagedata32[right] !== LAVA && gameImagedata32[right] !== BACKGROUND) skipDirectAdjacent = false;
	else if (down !== -1 && gameImagedata32[down] !== LAVA && gameImagedata32[down] !== BACKGROUND) skipDirectAdjacent = false;
	/*
	...
	if (!skipDirectAdjacent) {
		...
		if (random() < 15) {
			...
			if (right !== -1) {
				if (gameImagedata32[right] === FIRE) gameImagedata32[right] = BACKGROUND;
			}
		}
	}
	if (CS_ENABLED === true) {
		if (random() < 1) gameImagedata32[i] = ROCK;
	}
...
function ROCK_ACTION(x, y, i) {
	...
	if (HW_ENABLED === true) {
		if (random() < 1) gameImagedata32[i] = LAVA;
	}
...
function STEAM_ACTION(x, y, i) {
  ...
  /* Cold Snap modifier */
  if (CS_ENABLED === true) {
		if (random() < 10) gameImagedata32[i] = WATER;
	}
}
...
function CRYO_ACTION(x, y, i) {
	...
	for (yIter = yStart; yIter !== yEnd; yIter++) {
		...
		for (xIter = xStart; xIter !== xEnd; xIter++) {
			...
			if (borderingElem === WALL || borderingElem === BEDROCK || borderingElem === SPOUT || borderingElem === WAX || borderingElem === WELL || borderingElem === FUSE || borderingElem === PLANT || borderingElem === C4) {
				gameImagedata32[i] = CHILLED_ICE;
				return;
			}
...
function MYSTERY_ACTION(x, y, i) {
	if (particles.particleActive(MAGIC1_PARTICLE) || particles.particleActive(MAGIC2_PARTICLE)) {
		gameImagedata32[i] = BACKGROUND;
		return;
	}
	if (doGravity(x, y, i, true, 95)) return;
  ...
  if (borderingAdjacent(x, y, i, SPICE) !== -1) {
		gameImagedata32[i] = __mysteryElements[Math.floor(Math.random() * __mysteryElements.length)];
    gameImagedata32[idx] = __mysteryElements[Math.floor(Math.random() * __mysteryElements.length)];
		return;
	}
	...
	if (bordering(x, y, i, FIRE) !== -1) {
		for (var idx = MAX_IDX; idx !== 0; idx--) {
			...
			if (swapElem === WALL || swapElem === FIRE || swapElem === MYSTERY) continue;
...
function WET_SOIL_ACTION(x, y, i) {
	...
	if (random() < 5) {
		...
		if (random() < 35) return;
		if (aboveAdjacent(x, y, i, BACKGROUND) !== -1 && (belowAdjacent(x, y, i, SOIL) !== -1 || belowAdjacent(x, y, i, WALL) !== -1)) {
	...
	if (random() < 10) {
		...
		for (yLocsIter = 0; yLocsIter !== 3; yLocsIter++) {
			...
			for (xLocsIter = 0; xLocsIter !== 3; xLocsIter++) {
				...
				const idx = idxBase + xIter;
				const borderingElem = gameImagedata32[idx];
				if (borderingElem === ACID || borderingElem === BACKGROUND || borderingElem === BEDROCK || borderingElem === DEEPSAND || borderingElem === WATER || borderingElem === SALT_WATER || borderingElem === ICE || borderingElem === CHILLED_ICE || borderingElem === CRYO || borderingElem === FACTORY || borderingElem === VOID) continue;
				if (yIter !== y + 1) {
					gameImagedata32[idx] = BACKGROUND;
					return;
				}
				gameImagedata32[i] = BACKGROUND;
				if (borderingElem !== WALL || random() < 75) gameImagedata32[idx] = ACID;
				return;
			}
		}
	}
...
function BURNING_THERMITE_ACTION(x, y, i) {
	...
	for (iter = 0; iter !== 3; iter++) {
		...
		if (burnLoc === -1) continue;
		const elem = gameImagedata32[burnLoc];
		if (elem !== THERMITE && elem !== BURNING_THERMITE && elem !== LAVA && elem !== WALL) {
...
function HUMAN_ACTION(x, y, i) {
   ...
   */
  if (random() < 90) {
      if ((above(y, i, BACKGROUND) === -1) && (above(y, i, HUMAN) === -1)) {
    		walkAround(x, i, 40);
    	...
    	if ((adjacentLeft(x, i, BACKGROUND) === -1) && (adjacentLeft(x, i, HUMAN) === -1)) {
    		moveUp(y, i, 60);
    		moveLeft(x, i, 75);
        return;
    	}
...
function THANOS_ACTION(x, y, i) {
	...
	if ((borderingAdjacent(x, y, i, FIRE) !== -1) || (borderingAdjacent(x, y, i, FIRE_CURSE) !== -1)) {
		for (var idx = MAX_IDX; idx !== 0; idx--) {
			...
			const swapIdx = Math.floor(Math.random() * idx);
			const swapElem = gameImagedata32[swapIdx];
			if (swapElem === FIRE || swapElem === FIRE_CURSE || swapElem === THANOS || swapElem === BACKGROUND) continue;
			gameImagedata32[idx] = swapElem;
			gameImagedata32[swapIdx] = currElem;
		}
	}
...
function SPICE_ACTION(x, y, i) {
	...
	if ((borderingAdjacent(x, y, i, FIRE) !== -1) || (borderingAdjacent(x, y, i, FIRE_CURSE) !== -1) || (II_ENABLED === true)) {
		if (random() < 20) {
      particles.addActiveParticle(NITRO_PARTICLE, x, y, i);
    }
		if (random() < 15) {
      particles.addActiveParticle(NAPALM_PARTICLE, x, y, i);
    }
		if (random() < 10) {
      particles.addActiveParticle(C4_PARTICLE, x, y, i);
    }
		if (random() < 5) {
      particles.addActiveParticle(LAVA_PARTICLE, x, y, i);
    }
		if (random() < 10) {
      particles.addActiveParticle(METHANE_PARTICLE, x, y, i);
    }
		if (random() < 10) {
      particles.addActiveParticle(CHARGED_NITRO_PARTICLE, x, y, i);
    }
		if (random() < 10) {
      particles.addActiveParticle(CHARGED_CURSE_PARTICLE, x, y, i);
    }
		if (random() < 10) {
      particles.addActiveParticle(FIREWORK_PARTICLE, x, y, i);
    }
		if (random() < 10) {
      particles.addActiveParticle(CURSEWORK_PARTICLE, x, y, i);
    }
	}
  if (random() < 50) {
	  walkAround(x, i, 50);
...
function CLONE_ACTION(x, y, i) {
	...
	if (colorFire(x, y, i, 25, true)) return;
}
...
function REMOTE_C4_ACTION(x, y, i) {
	if ((REMOTE_BOMB === true) || (II_ENABLED === true)) {
		if (random() < 20) {
			if (!particles.addActiveParticle(C4_PARTICLE, x, y, i)) {
				gameImagedata32[i] = FIRE;
			}
...
function VOID_ACTION(x, y, i) {
	...
	for (yIter = yStart; yIter !== yEnd; yIter++) {
		...
		for (xIter = xStart; xIter !== xEnd; xIter++) {
			...
			if (borderingElem !== VOID) {
        if (random() < 50) {
          return gameImagedata32[idx] = BACKGROUND;
			  }
      }
		}
	}
...
function belowAdjacent(x, y, i, type) {
	...
	const belowLeftMatch = x !== 0 && gameImagedata32[belowLeftSpot] === type ? belowLeftSpot : -1;
	const belowRightSpot = belowSpot + 1;
	const belowRightMatch = x !== MAX_X_IDX && gameImagedata32[belowRightSpot] === type ? belowRightSpot : -1;
...
function aboveAdjacent(x, y, i, type) {
	...
	const aboveLeftMatch = x !== 0 && gameImagedata32[aboveLeftSpot] === type ? aboveLeftSpot : -1;
	const aboveRightSpot = aboveSpot + 1;
	const aboveRightMatch = x !== MAX_X_IDX && gameImagedata32[aboveRightSpot] === type ? aboveRightSpot : -1;
...
function adjacent(x, i, type) {
	const leftSpot = i - 1;
	const rightSpot = i + 1;
	const leftMatch = x !== 0 && gameImagedata32[leftSpot] === type ? leftSpot : -1;
	const rightMatch = x !== MAX_X_IDX && gameImagedata32[rightSpot] === type ? rightSpot : -1;
	return __pickRandValid(leftMatch, rightMatch);
}
/* Checks left. */
function adjacentLeft(x, i, type) {
	const leftSpot = i - 1;
	const leftMatchCheck = x !== 0 && gameImagedata32[leftSpot] === type ? leftSpot : -1;
	return leftMatchCheck;
}
/* Checks right. */
function adjacentRight(x, i, type) {
	const rightSpot = i + 1;
	const rightMatchCheck = x !== MAX_X_IDX && gameImagedata32[rightSpot] === type ? rightSpot : -1;
	return rightMatchCheck;
}
...
function moveLeft(x, i, chance) {
	...
	var newI = adjacentLeft(x, i, BACKGROUND);
	if (newI !== -1) {
		gameImagedata32[newI] = gameImagedata32[i];
		gameImagedata32[i] = BACKGROUND;
		return true;
	} else {
		gameImagedata32[newI] = BACKGROUND;
	}
	return false;
}
/*Moves an element up.*/
function moveUp(y, i, chance) {
	if (random() >= chance) return false;
	if (y === MAX_Y_IDX) {
		gameImagedata32[i] = BACKGROUND;
		return true;
	}
	var newI = above(y, i, BACKGROUND);
	if (newI !== -1) {
		gameImagedata32[newI] = gameImagedata32[i];
		gameImagedata32[i] = BACKGROUND;
		return true;
...
function moveRight(x, i, chance) {
	...
	var newI = adjacentRight(x, i, BACKGROUND);
	if (newI !== -1) {
		gameImagedata32[newI] = gameImagedata32[i];
		gameImagedata32[i] = BACKGROUND;
		return true;
...
function walkAround(x, i, chance) {
	...
	var newILeft = adjacentLeft(x, i, BACKGROUND);
	var newIRight = adjacentRight(x, i, BACKGROUND);
	var newIDirect = __pickRandValid(newILeft, newIRight);
	var oldI = gameImagedata32[i];
	var newI = __pickRandValid(newIDirect, oldI);
	if ((newI !== -1) && (newI === newIDirect)) {
		gameImagedata32[newI] = gameImagedata32[i];
		gameImagedata32[i] = BACKGROUND;
		return true;
	} else {
...
function doRise(x, y, i, riseChance, adjacentChance) {
	...
	if (newI === -1 && random() < adjacentChance) newI = adjacent(x, i, BACKGROUND);
	if (newI !== -1) {
		gameImagedata32[newI] = gameImagedata32[i];
		gameImagedata32[i] = BACKGROUND;
		return true;
...
function doDensityLiquid(x, y, i, heavierThan, sinkChance, equalizeChance) {
	...
	if (newI === -1 && random() < equalizeChance) newI = adjacent(x, i, heavierThan);
	if (newI === -1) return false;
	gameImagedata32[newI] = gameImagedata32[i];
	gameImagedata32[i] = heavierThan;
	return true;
...
/*Semi-copy of __doBorderBurn. Fun fact, CORRUPT was my first element, and it was originally going to be a CRYO clone that would cool and harden LAVA.*/
function __doCorruption(x, y, i) {
	if (random() < 5) {
		if (bordering(x, y, i, CORRUPT) !== -1) {
			...
			if (x !== 0) gameImagedata32[i - 1] = CORRUPT;
			if (x !== MAX_X_IDX) gameImagedata32[i + 1] = CORRUPT;
			gameImagedata32[i] = CORRUPT;
			return;
		}
	}
}
...
function __doGunpowderExplosion(x, y, i) {
	...
	if (y - 2 >= 0) {
		...
		if (gameImagedata32[twoUp] !== GUNPOWDER || random() < 50) gameImagedata32[twoUp] = FIRE;
	}
	if (y + 2 >= 0) {
		const twoDown = i + 2 * width;
		if (gameImagedata32[twoDown] !== GUNPOWDER || random() < 50) gameImagedata32[twoDown] = FIRE;
	}
	if (x - 2 >= 0) {
		const twoLeft = i - 2;
		if (gameImagedata32[twoLeft] !== GUNPOWDER || random() < 50) gameImagedata32[twoLeft] = FIRE;
	}
	if (x + 2 >= 0) {
		const twoRight = i + 2;
		if (gameImagedata32[twoRight] !== GUNPOWDER || random() < 50) gameImagedata32[twoRight] = FIRE;
	}
}

function colorFire(x, y, i, chance, HW_Influence) {
	/* flame out (unless in contact with element that needs to retain fire) */
	if ((HW_ENABLED === true) && (HW_Influence === true)) {
		flameOutChance = 35;
	} else {
		flameOutChance = chance;
	}
	if (random() < flameOutChance) {
		var flameOut = true;
		const xStart = Math.max(x - 1, 0);
		...
		if (flameOut && bordering(x, y, i, BACKGROUND) !== -1) flameOut = true;
		if (flameOut) {
			gameImagedata32[i] = BACKGROUND;
			return;
		}
...
function fireRise(x, y, i, fireType) {
	/* rising fire */
	if (random() < 50) {
		const riseLoc = above(y, i, BACKGROUND);
		if (riseLoc !== -1) {
			gameImagedata32[riseLoc] = fireType;
			return;
		}
	}
}

function doTransform(x, y, i, transformBy, transformInto, transformChance, consumeChance) {
	const rand = random();
...
function doProducer(x, y, i, produce, overwriteAdjacent, chance) {
	...
	const left = i - 1;
	const right = i + 1;
	if (y !== 0 && (overwriteAdjacent || gameImagedata32[up] === BACKGROUND)) gameImagedata32[up] = produce;
	if (y !== MAX_Y_IDX && (overwriteAdjacent || gameImagedata32[down] === BACKGROUND)) gameImagedata32[down] = produce;
	if (x !== 0 && (overwriteAdjacent || gameImagedata32[left] === BACKGROUND)) gameImagedata32[left] = produce;
	if (x !== MAX_X_IDX && (overwriteAdjacent || gameImagedata32[right] === BACKGROUND)) gameImagedata32[right] = produce;
}
/*
...
function doDensityGas(x, y, i, chance) {
	...
	if (gasPermeable(aboveElem)) swapSpot = aboveSpot;
		 ...
		 */
		if (aboveLeftElem !== aboveElem && gasPermeable(aboveLeftElem)) swapAboveLeft = aboveLeft;
		if (aboveRightElem !== aboveElem) {
			if (swapAboveLeft !== -1 && aboveLeftElem === aboveRightElem) swapAboveRight = aboveRight;
	...
	if (swapSpot === -1 && x !== 0 && x !== MAX_X_IDX && y !== MAX_Y_IDX) {
		...
		if (gasPermeable(leftElem) && gameImagedata32[i - 1 + width] !== gasElem) {
			...
			if (gasPermeable(rightElem) && gameImagedata32[i + 1 + width] !== gasElem) swapSpot = i + 1;
		}
	}
