...
var REMOTE_BOMB;
/* 
 * I put modifiers here since at its core
 * they're more or less OVERWRITE_ENABLED copies.
 */
var MS_ENABLED;
var HW_ENABLED;
var CS_ENABLED;
...
class Cursor {
	...
	canvasCursorLeave(getOuterCoords) {
		...
		Cursor.interpolateCursorBorderPosition(
			[this.prevX, this.prevY], outerCoords);
	...
	drawStroke() {
		 ...
		 * modify it.
		 */
		const colorString = color !== 0xff000000 ? "rgba(" + r + "," + g + "," + b + ", 1)" : "rgba(1, 0, 0, 1)";
		...
		if (userstrokeCanvas.width < userstroke_width) userstrokeCanvas.width = userstroke_width;
		if (userstrokeCanvas.height < userstroke_height) userstrokeCanvas.height = userstroke_height;
		...
		const strokeImageData = userstrokeCtx.getImageData(0, 0, userstroke_width, userstroke_height);
		...
		for (y = yStart; y !== yTerminate; y++) {
			...
			for (x = xStart; x !== xTerminate; x++) {
				...
				if (strokeImageData32[x + offset_relative] !== 0xff000000) {
					...
					if (overwrite || gameImagedata32[absIdx] === BACKGROUND) gameImagedata32[absIdx] = color;
				}
			}
		}
	}
...
class Mouse extends Cursor {
	...
	handleShift() {
		...
		if (direction === Mouse.HORIZONTAL) {
		...
		} else if (direction === Mouse.DIAGONAL_DOWN || direction === Mouse.DIAGONAL_UP) {
			this.prevX = this.shiftStartX;
			this.prevY = this.shiftStartY;
			...
			if (distance(xAdjusted, this.y, this.shiftStartX, this.shiftStartY) > distance(this.x, yAdjusted, this.shiftStartX, this.shiftStartY)) {
				this.x = xAdjusted;
			} else {
				this.y = yAdjusted;
			}
