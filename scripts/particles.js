...
function NAPALM_PARTICLE_ACTION(particle) {
	...
	if (particle.actionIterations > particle.maxIterations) particles.makeParticleInactive(particle);
}
...
function LAVA_PARTICLE_INIT(particle) {
	...
	if (random() < 75 && Math.abs(HALF_PI - angle) < EIGHTEENTH_PI) angle += EIGHTEENTH_PI * (angle > HALF_PI ? 1 : -1);
...
function LAVA_PARTICLE_ACTION(particle) {
	...
	const iterations = particle.actionIterations;
	particle.x += particle.xVelocity;
	particle.y = particle.initY + particle.initYVelocity * iterations + (particle.yAcceleration * iterations * iterations) / 2;
	offscreenParticleCtx.lineTo(particle.x, particle.y);
	offscreenParticleCtx.stroke();
	...
	if (random() < 25) {
		...
		particle.yVelocity = particle.initYVelocity + particle.yAcceleration * iterations;
		...
		} else if (touchingColor === ICE || touchingColor === CHILLED_ICE || touchingColor === CRYO) {
...
function FIREWORK_PARTICLE_INIT(particle) {
	...
	/* Make it harder for the angle to be steep */
	var angle = QUARTER_PI + Math.random() * HALF_PI;
	if (random() < 55 && Math.abs(HALF_PI - angle) < EIGHTEENTH_PI) angle += EIGHTEENTH_PI * (angle > HALF_PI ? 1 : -1);
	particle.xVelocity = (1 + Math.random() * 3) * Math.cos(angle);
	particle.yVelocity = (-4 * Math.random() - 3) * Math.sin(angle);
	particle.initYVelocity = particle.yVelocity;
	particle.yAcceleration = 0.06;
...
function MAGIC1_PARTICLE_INIT(particle) {
	...
	for (i = 1; i !== numSpokes; i++) {
		 ...
		 */
		const newSpoke = particles.addActiveParticle(UNKNOWN_PARTICLE, particle.x, particle.y, particle.i);
		if (!newSpoke) break;
		/*
...
function MAGIC2_PARTICLE_INIT(particle) {
	...
	particle.magic_2_max_radius = Math.sqrt(width * width + height * height) / 2 + particle.size;
...
function MAGIC2_PARTICLE_ACTION(particle) {
	...
	offscreenParticleCtx.lineCap = "round";
	offscreenParticleCtx.moveTo(particle.x, particle.y);
	const newTheta = particle.magic_2_theta + particle.magic_2_speed / particle.magic_2_radius;
	particle.magic_2_theta = newTheta;
	const newRadius = (particle.magic_2_theta / TWO_PI) * particle.magic_2_radius_spacing;
	...
	offscreenParticleCtx.lineTo(particle.x, particle.y);
	offscreenParticleCtx.stroke();
	if (newRadius > particle.magic_2_max_radius) particles.makeParticleInactive(particle);
}
...
class Tree1 extends TreeType {
	...
	/** @nocollapse */
	static branchAngles(treeParticle) {
		const branchAngle = (EIGHTH_PI + Math.random() * EIGHTH_PI) * treeParticle.branchDirection;
...
function TREE_PARTICLE_ACTION(particle) {
	...
	if (iterations >= particle.nextBranch) {
		...
		const leafBranch = particle.color === LEAF || particle.branches === particle.maxBranches;
		...
		for (var i = 0; i < numBranches; i++) {
			const b = particles.addActiveParticle(TREE_PARTICLE, particle.x, particle.y, particle.i);
			if (!b) break;
			b.generation = particle.generation + 1;
			b.maxBranches = Math.max(0, particle.maxBranches - 1);
			b.branchSpacing = particle.branchSpacing * treeInfo.branchSpacingFactor(particle);
...
function HUMAN_PARTICLE_INIT(particle) {
	...
	/* Make it harder for the angle to be steep */
	var angle = QUARTER_PI + Math.random() * HALF_PI;
	if (random() < 90 && Math.abs(HALF_PI - angle) < EIGHTEENTH_PI) angle += EIGHTEENTH_PI * (angle > HALF_PI ? 1 : -1);
...
function initParticles() {
	if (__particleInit.length !== __particleActions.length) throw "Particle arrays must be same length";
...
function updateParticles() {
	...
	const particleImageData = offscreenParticleCtx.getImageData(0, 0, canvasWidth, canvasHeight);
